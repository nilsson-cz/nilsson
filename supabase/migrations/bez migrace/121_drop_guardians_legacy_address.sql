-- =============================================================================
-- Migrace 121 — M6: drop legacy adresních sloupců na guardians
-- Datum: 2026-09-21 (idempotentní)
-- Prerekvizita: 115 (addresses), 116/119 (backfill do addresses HOTOVÝ),
--   120 (enrollment_migrate_to_student).
-- PRD: Nilsson_documentation/daily_notes/PRD-adresni-model-2026-09-20.md (M6, §9 Q3)
--
-- Účel: jednotný adresní model je nasazen a všichni konzumenti (studijní smlouva,
--   katalogový list, úrazy, karta žáka) čtou adresy už jen z tabulky `addresses`.
--   Legacy sloupce na `guardians` (address_street/city/zip/country/delivery z matriky
--   + address_ruian_kod/validated_at z migrace 037) jsou tím redundantní — všechny
--   kompletní adresy byly zbackfillované do addresses (119: 22 zástupců + 27 žáků).
--
-- Kroky:
--   1) enrollment_migrate_to_student přestane zapisovat guardian adresu do
--      guardians.address_* (píše ji jen do addresses, což už dělá od M4a/M4b).
--   2) DROP COLUMN legacy adresních sloupců guardians.
--
-- ⚠️ DESTRUKTIVNÍ a NEVRATNÉ. Před spuštěním ověř, že addresses pokrývá potřebu:
--   -- kolik zástupců MÁ v guardians adresu, ale NEMÁ ji v addresses (mělo by být 0):
--   SELECT count(*) FROM guardians g
--   WHERE g.address_street IS NOT NULL AND g.address_city IS NOT NULL AND g.address_zip IS NOT NULL
--     AND NOT EXISTS (SELECT 1 FROM addresses a WHERE a.guardian_id = g.id AND a.typ = 'trvale');
--
...
-- -----------------------------------------------------------------------------
-- 1. enrollment_migrate_to_student: guardian adresa už jen do addresses
--    (tělo = kopie 120 bez guardians.address_* v INSERTu guardians).
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  -- --- 1b) addresses: trvalé + kontaktní bydliště dítěte ---
  ...
  LOOP
    ...
    -- guardian TRVALÉ bydliště → addresses
    ...
    -- guardian KONTAKTNÍ bydliště → addresses (M4b)
    IF v_guardian_row.address_kontaktni_cislo IS NOT NULL THEN
      ...
      VALUES (
        ...
        v_guardian_row.address_kontaktni_ruian_kod, v_guardian_row.address_kontaktni_validated_at, 'CZ'
      )
      ON CONFLICT (guardian_id, typ) WHERE guardian_id IS NOT NULL DO NOTHING;
    END IF;
...
-- -----------------------------------------------------------------------------
-- 2. DROP legacy adresních sloupců na guardians
--    (matrika: street/city/zip/country/delivery; 037: ruian_kod/validated_at)
-- -----------------------------------------------------------------------------
ALTER TABLE guardians DROP COLUMN IF EXISTS address_street;
ALTER TABLE guardians DROP COLUMN IF EXISTS address_city;
ALTER TABLE guardians DROP COLUMN IF EXISTS address_zip;
ALTER TABLE guardians DROP COLUMN IF EXISTS address_country;
ALTER TABLE guardians DROP COLUMN IF EXISTS address_delivery;
ALTER TABLE guardians DROP COLUMN IF EXISTS address_ruian_kod;
ALTER TABLE guardians DROP COLUMN IF EXISTS address_validated_at;

COMMIT;

-- =============================================================================
-- Po migraci: `npm run db:types`. Ověření, že sloupce zmizely:
--   SELECT column_name FROM information_schema.columns
--   WHERE table_name = 'guardians' AND column_name LIKE 'address_%';
--   -- očekává se prázdný výsledek.
-- =============================================================================
