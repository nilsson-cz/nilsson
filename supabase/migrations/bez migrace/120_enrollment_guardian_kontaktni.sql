-- =============================================================================
-- Migrace 120 — Kontaktní adresa zástupce v přijímacím řízení (M4b, schéma)
-- Datum: 2026-09-21 (idempotentní)
-- Prerekvizita: 037 (enrollment_guardians), 118 (enrollment_migrate_to_student),
--   115 (addresses).
-- PRD: Nilsson_documentation/daily_notes/PRD-adresni-model-2026-09-20.md (M4b)
--
-- Účel: umožnit zachytit KONTAKTNÍ adresu zákonného zástupce v přijímacím
--   formuláři (dnes má enrollment_guardians jen trvalé address_*). Sloupce jsou
--   nepovinné (kontaktní adresa se zadává, jen když se liší od trvalé). Povinnost
--   TRVALÉ adresy zástupce (§9 Q1) vynucuje wizard (frontend), ne DB.
--
--   + enrollment_migrate_to_student při přijetí zapíše i guardian KONTAKTNÍ do
--     addresses (vedle guardian trvalé z migrace 118).
--
-- Po spuštění: `npm run db:types` (přibudou sloupce enrollment_guardians) — pak
--   navazuje kód wizardu (saveEnrollmentOwner + AddressField).
--
...
-- -----------------------------------------------------------------------------
-- 1. Kontaktní adresa zástupce (nepovinná) na enrollment_guardians
-- -----------------------------------------------------------------------------
ALTER TABLE enrollment_guardians ADD COLUMN IF NOT EXISTS address_kontaktni_obec         text;
ALTER TABLE enrollment_guardians ADD COLUMN IF NOT EXISTS address_kontaktni_ulice        text;
ALTER TABLE enrollment_guardians ADD COLUMN IF NOT EXISTS address_kontaktni_cislo        text;
ALTER TABLE enrollment_guardians ADD COLUMN IF NOT EXISTS address_kontaktni_psc          text;
ALTER TABLE enrollment_guardians ADD COLUMN IF NOT EXISTS address_kontaktni_ruian_kod    text;
ALTER TABLE enrollment_guardians ADD COLUMN IF NOT EXISTS address_kontaktni_validated_at timestamptz;

-- -----------------------------------------------------------------------------
-- 2. enrollment_migrate_to_student: zapiš i guardian KONTAKTNÍ do addresses
--    (tělo = kopie 118 + nový blok guardian kontaktní v guardian loopu).
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    ...
    -- guardian KONTAKTNÍ bydliště → addresses (nové, M4b)
...
--   WHERE table_name='enrollment_guardians' AND column_name LIKE 'address_kontaktni%';
-- =============================================================================
